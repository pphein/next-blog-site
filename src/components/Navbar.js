    ...
    return (
        <header className="cursor-pointer container mx-auto px-5 flex justify-between py-4 items-center z-50">
            ...
            transition-all duration-300 mt-[56pxz lg:mt-0 [49] bg-primary lg:bg-transparent flex flex-col w-full lg:w-auto lg:flex-row justify-center lg:justify-end fixed top-0 bottom-0 -right-full lg:static flex gap-x-9 items-center`}>
                <ul className="z-50 gap-y-5 items-center flex gap-x-5 flex-col lg:flex-row font-semibold gap-x-2">
                    <li>
                        <Link className="text-orange-300 text-2xl font-bold hover:bg-orange-700 p-2 border-md hover:text-white transition-all duration-300"
                            href={'/'}>
                            Home
                        </Link>
                    </li>
                    ...
                    session?.user
                        ? (
                            <>
                                <Link
                                    href='/create-post'
                                    className="px-6 py-2.5 rounded-md bg-green-500 mt-3 text-white hover:bg-indigo-500 hover:text-white transition-all duration-300"
                                    onClick={navVisibilityHandler}
                                >
                                    Create Post
                                </Link>
                                <button
                                    onClick={() => { signOut(), navVisibilityHandler() }}
                                    className="px-6 py-2.5 rounded-md bg-green-500 mt-3 text-white hover:bg-indigo-500 hover:text-white transition-all duration-300"
                                >
                                    Logout
                                </button>
                            ...
                            <>
                                <Link
                                    href='/login'
                                    className="px-6 py-2.5 rounded-md bg-green-500 mt-3 text-white hover:bg-blue-500 hover:text-white transition-all duration-300"
                                    onClick={navVisibilityHandler}
                                >
                                    Login
                                </Link>
